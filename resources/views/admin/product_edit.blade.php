...
<section class="text-gray-600">
    <div class="container px-5 py-24 mx-auto">
        <div class="lg:w-2/4 w-full mx-auto overflow-auto">
            ...
            <form enctype="multipart/forma-data" method="POST" action="{{ route('admin.product.update', $products->id) }}">
                @csrf
                @method('PUT')
                <div class="flex flex-wrap">
                    <div class="p-2 w-1/2">
                        <div class="relative">
                            ...
                            <input value="{{ old('name'),$products->name }}" type="text" id="name" name="name" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                        @error('name')
                    ...
                    <div class="p-2 w-1/2">
                        <div class="relative">
                            ...
                            <input value="{{ old('price'), $products->price }}" type="text" id="price" name="price" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
                        </div>
                        @error('price')
                        <div class="text-red-400 text-sm">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="p-2 w-1/2">
                        <div class="relative">
                            ...
                            <input value="{{ old('stock'), $products->stock }}" type="text" id="stock" name="stock" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                        @error('stock')
                        <div class="text-red-400 text-sm">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="p-2 w-full">
                        <img src="{{\Illuminate\Support\Facades\Storage::url($products->cover) }}" alt="">

                        <a href="{{ route('admin.product.destroyImage', $products->id) }}">Deletar imagem</a>
                    </div>
                    @endif
